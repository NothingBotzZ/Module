...
if (!isCmd && isGroup && checkResponGroup(from, chats, db_respon_group)) {
            fadly.sendMessage(from, { text: sendResponGroup(from, chats, db_respon_group) }, {
                quoted: msg
            })
        }
...
Games Draw, Tidak Ada Pemenang`
        ...
        case prefix+'sewabot': case prefix+'sewa':
            ...
            let butSewa = [
            { urlButton: { displayText: `Click Here!`, url : `https://wa.me/6285380779466` } }
            ]
            buttonWithText(from, `*🧧SEWA  ${botName}🧧*
...
*╰────❏*
*╭─[ FITUR BOT ]*
*│*❏  ADDLIST (TAMBAH/UPDATE LIST)
*│*❏  HIDETAG (TAG ALL MEMBER)
*│*❏  ANTI LINK 
*│*❏  WELCOME & LEAVE
*╰────❏*`, footxt, butSewa)
            break
        case prefix+'listpremium': case prefix+'listprem':
            ...
            for (let i of premium) {
                ...
                if (i.expired === 'PERMANENT') {
                    ...
                    txt += `*Expire :* ${cekvip.days} Hari ${cekvip.hours} Jam ${cekvip.minutes} Menit ${cekvip.seconds} Detik\n\n`
                }
            }
            ...
            if (isImage || isQuotedImage) {
                ...
                fetch('https://telegra.ph/upload', {
                    ...
                    .then((json) => {
                        ...
                        reply(`「 SUKSES 」menambah List menu *${args1}*`)
            ...
            if (isImage || isQuotedImage) {
                ...
                fetch('https://telegra.ph/upload', {
                    ...
                    .then((json) => {
                        ...
                        reply(`「 SUKSES 」update List menu : *${args1}*`)
                        if (fs.existsSync(media)) fs.unlinkSync(media)
                    })
            } else {
...
_Silahkan Pilih Format yang ada dibawah_`
        ...
        case prefix+'tebakgambar': case prefix+'tg':
		    if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
		    if (isPlayGame(from, tebakgambar)) return fadly.reply(from, `Masih ada game yang belum diselesaikan`, tebakgambar[getGamePosi(from, tebakgambar)].msg)
		...
		case prefix+'kuis':
		    if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
		    if (isPlayGame(from, kuis)) return fadly.reply(from, `Masih ada game yang belum diselesaikan`, kuis[getGamePosi(from, kuis)].msg)
        ...
        case prefix+'tebaklagu': case prefix+'tl':
            if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
            if (isPlayGame(from, tebaklagu)) return fadly.reply(from, `Masih ada game yang belum diselesaikan`, tebaklagu[getGamePosi(from, tebaklagu)].msg)
        ...
        case prefix+'family100':
            if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
            if (isPlayGame(from, family100)) return fadly.reply(from, `Masih ada soal yang belum diselesaikan`, family100[getGamePosi(from, family100)].msg)
...
case prefix + 'delrespon':
            ...
            case prefix + 'set':
                ...
                try {
                    ...
                    reply(`「 SUKSES 」Sukses Update respon *${azu}*`)
                } catch (err) {
                    reply(mess.error.api)
                    console.log(color('[ERROR]', 'red'), err)
                }
            ...
            case prefix + 'addrespon':
                ...
                try {
                    ...
                    reply(`「 SUKSES 」Menambah respon *${aza}*`)
                } catch (err) {
                    reply(mess.error.api)
                    console.log(color('[ERROR]', 'red'), err)
                }
